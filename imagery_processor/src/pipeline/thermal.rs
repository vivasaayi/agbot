...
use crate::{ThermalArgs, OutputFormat, TemperatureUnit};
...
async fn process_one(metadata_file: &PathBuf, args: &ThermalArgs) -> AgroResult<()> {
    ...
    let t_path = image.file_paths.get(&args.thermal_band)
        .ok_or_else(|| shared::error::AgroError::Processing(format!("Thermal band '{}' not found", args.thermal_band)))?;
    ...
    let is_landsat_b10 = args.thermal_band.eq_ignore_ascii_case("B10") || t_path.to_lowercase().contains("_b10");
    let is_landsat_b11 = args.thermal_band.eq_ignore_ascii_case("B11") || t_path.to_lowercase().contains("_b11");
    let dyn_img = image::open(t_path)
        .map_err(|e| shared::error::AgroError::Processing(format!("Failed to load thermal band: {}", e)))?;
    let is_u16 = matches!(dyn_img.color(), image::ColorType::L16 | image::ColorType::La16 | image::ColorType::Rgb16 | image::ColorType::Rgba16);
    let t_img_u16 = if is_u16 { Some(dyn_img.to_luma16()) } else { None };
    let t_img_u8 = if is_u16 { None } else { Some(dyn_img.to_luma8()) };
    let (w, h) = if let Some(ref im) = t_img_u16 { im.dimensions() } else { t_img_u8.as_ref().unwrap().dimensions() };
    ...
    let (ml_def, al_def, k1_def, k2_def, lambda_def) = if is_landsat_b11 {
        ...
        // Generic defaults
        (args.ml.unwrap_or(0.0003342), args.al.unwrap_or(0.1), args.k1.unwrap_or(774.8853), args.k2.unwrap_or(1321.0789), args.lambda_um)
    };
    ...
    let lambda = if is_landsat_b10 || is_landsat_b11 { lambda_def } else { args.lambda_um }; // micrometers
    ...
    for y in 0..h {
        for x in 0..w {
            if !mask_img.as_ref().map_or(true, |m| m.get_pixel(x,y)[0] != 0) {
                out_vis.put_pixel(x, y, image::Luma([0]));
                continue;
            }
            let dn: f32 = if let Some(ref im16) = t_img_u16 { im16.get_pixel(x, y)[0] as f32 } else { t_img_u8.as_ref().unwrap().get_pixel(x,y)[0] as f32 };
            ...
            let tb = if l > 0.0 { k2 / ((k1 / l).ln_1p()) } else { f32::NAN };
    ...
    let mean_t = if valid_count > 0 { (sum_t / valid_count as f64) as f32 } else { f32::NAN };
    ...
    let out_path = match args.out_format {
        OutputFormat::Png => {
            ...
            out_vis.save(&p)
                .map_err(|e| shared::error::AgroError::Processing(format!("Failed to save thermal image: {}", e)))?;
            p
        }
        OutputFormat::Geotiff => {
            #[cfg(feature = "gdal-io")]
            {
                ...
                crate::io::gdal_util::write_f32_geotiff(p.to_string_lossy().as_ref(), &out_f32, w as usize, h as usize, Some(-9999.0))
