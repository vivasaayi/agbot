...
#[cfg(feature = "gdal-io")]
pub fn read_first_band_as_f32(
    path: &str,
) -> Result<(usize, usize, Vec<f32>, Option<f64>), gdal::errors::GdalError> {
...
#[cfg(feature = "gdal-io")]
pub fn read_first_band_as_f32_resampled(
    path: &str,
    out_w: usize,
    out_h: usize,
) -> Result<(usize, usize, Vec<f32>, Option<f64>), gdal::errors::GdalError> {
...
pub fn write_f32_geotiff(
    ...
    let mut ds = drv.create_with_band_type::<f32>(path, width as isize, height as isize, 1)?;
    {
        ...
        if let Some(nd) = nodata {
            let _ = band.set_no_data_value(nd);
        }
    }
    Ok(())
}
