...
impl NdviAnalysisProcessor {
    ...
    pub async fn process_ndvi_request(
        &mut self,
        ...
        tracing::info!(
            "Processed NDVI analysis for request {} in {}ms",
            request.id,
            processing_time
        );
        Ok(result)
    ...
    fn extract_vegetation_indices(
        &self,
        ndvi_map: &[f32],
        request: &NdviAnalysisRequest,
    ) -> Result<Vec<VegetationIndex>> {
    ...
    fn pixel_to_coordinates(
        ...
        let lat_range =
            request.georeference_info.top_left_lat - request.georeference_info.bottom_right_lat;
        let lon_range =
            request.georeference_info.bottom_right_lon - request.georeference_info.top_left_lon;

        let lat = request.georeference_info.top_left_lat
            - (row as f64 / request.image_height as f64) * lat_range;
        let lon = request.georeference_info.top_left_lon
            + (col as f64 / request.image_width as f64) * lon_range;
    ...
    fn calculate_statistics(
        &self,
        ndvi_map: &[f32],
        _request: &NdviAnalysisRequest,
    ) -> Result<NdviStatistics> {
        let valid_pixels: Vec<f32> = ndvi_map
            .iter()
            .filter(|&&val| val >= 0.0)
            .copied()
            .collect();
        ...
        let variance = valid_pixels.iter().map(|x| (x - mean).powi(2)).sum::<f32>()
            / valid_pixels.len() as f32;
        let std_deviation = variance.sqrt();

        let min_ndvi = *sorted_pixels.first().unwrap();
        let max_ndvi = *sorted_pixels.last().unwrap();

        let vegetation_pixels = valid_pixels
            .iter()
        ...
        let healthy_pixels = valid_pixels
            .iter()
    ...
    pub async fn analyze(
        ...
        parameters: &super::ProcessingParameters,
...
mod tests {
    ...
    fn test_vegetation_classification() {
        ...
        assert_eq!(
            processor.classify_vegetation_type(-0.1),
            VegetationType::Water
        );
        assert_eq!(
            processor.classify_vegetation_type(0.05),
            VegetationType::BareGround
        );
        assert_eq!(
            processor.classify_vegetation_type(0.3),
            VegetationType::Grass
        );
        assert_eq!(
            processor.classify_vegetation_type(0.7),
            VegetationType::Crop
        );
        assert_eq!(
            processor.classify_vegetation_type(0.9),
            VegetationType::Forest
        );
    }
}
