...
- `GET /api/stac/collections` and `GET /api/stac/search` — the internal STAC
  ...
  reprojection yet).
- `GET /api/catalog/products/:product_id/tiles/:z/:x/:y.png` — **true Web
  Mercator XYZ tiles** for GeoTIFF-backed catalog products (STAC asset
  `tiles_web`), rendered server-side from the product's UTM grid with a
  deterministic per-kind colormap. Used directly as a MapLibre `raster`
  source with `bounds` from the item bbox; preferred whenever present.
- `GET /api/scenes/:scene_id/products/:kind/tiles/:z/:x/:y.png` — legacy
  product tiles. **These tiles are scene-local, not Web Mercator**: zoom `z`
  splits the product image itself into `2^z x 2^z` equal pixel tiles. The UI
  therefore stitches all tiles at a fixed zoom (z=2, 1024x1024) into a canvas
  and places it as a MapLibre `image` source positioned by the STAC item bbox
