use crate::app_state::{AppMode, GlobeSearchState, SelectedRegion};
use crate::earth_textures::{
    check_texture_loading, load_earth_textures, update_earth_material_with_textures, EarthTextures,
};
    ...
    commands.spawn((
        Camera3dBundle {
            camera: Camera {
                order: 1,
                ..default()
            }, // ensure higher priority than default scene camera
...
fn zoom_globe(mut globe_state: ResMut<GlobeState>, mut scroll_events: EventReader<MouseWheel>) {
...
fn update_selection_marker(
    ...
    if selected_region.is_changed() {
        ...
        if selected_region.center_lat != 0.0 || selected_region.center_lon != 0.0 {
            ...
            info!(
                ...
                selected_region.center_lat,
                selected_region.center_lon,
                sphere_pos.x,
                sphere_pos.y,
                sphere_pos.z
            );
    ...
    else if globe_state.is_changed()
        && (selected_region.center_lat != 0.0 || selected_region.center_lon != 0.0)
    {
...
fn handle_search_animation(
    ...
    let current_lat = search_state.start_lat
        + (search_state.target_lat - search_state.start_lat) * eased_progress as f64;
    let current_lon = search_state.start_lon
        + (search_state.target_lon - search_state.start_lon) * eased_progress as f64;

    // Interpolate zoom
    let current_zoom =
        globe_state.zoom + (search_state.target_zoom - globe_state.zoom) * eased_progress;
    ...
    info!(
        "🌍 ROTATION DEBUG: Rotating globe for location ({:.4}°, {:.4}°) → Y: {:.3}, X: {:.3}",
    ...
    if progress >= 1.0 {
        ...
        info!(
            "Animation completed. Arrived at: {:.4}, {:.4}",
            search_state.target_lat, search_state.target_lon
        );
    }
}
