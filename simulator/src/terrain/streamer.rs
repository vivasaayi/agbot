...
pub fn terrain_streamer_system(
    ...
    let Ok(cam_t) = camera_q.get_single() else {
        return;
    };
    ...
    for (_e, t) in existing_tiles.iter() {
        existing_set.insert((t.x, t.z));
    }
...
pub fn update_terrain_lod(
    ...
    for (t, mat_h, tile) in q.iter_mut() {
        ...
        if let Some(mat) = materials.get_mut(&*mat_h) {
            mat.base_color = Color::hsl(hue, 0.5, light);
        }
    }
}
