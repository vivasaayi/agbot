...
impl Plugin for HudPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, setup_hud)
            .add_systems(Update, (update_hud_elements, toggle_hud_visibility));
    }
}
            ...
            parent
                .spawn((
                    NodeBundle {
                        ...
                        background_color: Color::srgba(0.0, 0.0, 0.0, 0.7).into(),
                        border_color: Color::srgb(0.5, 0.0, 1.0).into(),
                        ..default()
                    },
                    HudElement {
...
fn update_hud_elements(
    _drone_query: Query<&crate::components::Drone>,
            ...
            if let Ok(mut text) = text_query.get_mut(child) {
                match hud_element.element_type {
                    ...
                    HudElementType::GpsStatus => {
                        ...
                        text.sections[0].value = if gps_ok {
                            "GPS: OK".to_string()
                        } else {
                            "GPS: WAIT".to_string()
                        };
                        text.sections[0].style.color = if gps_ok {
                            Color::srgb(0.0, 1.0, 0.0)
                        } else {
                            Color::srgb(1.0, 1.0, 0.0)
                        };
