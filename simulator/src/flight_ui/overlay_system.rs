use crate::flight_ui::{AppState, NotificationLevel, UIOverlayState};
use bevy::prelude::*;
...
impl Plugin for OverlaySystemPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Update, notification_system);
    }
}
    ...
    for (index, notification) in overlay_state.notification_queue.iter().enumerate() {
        ...
        egui::Window::new("")
            ...
            .anchor(
                ...
                egui::Vec2::new(-20.0, 20.0 + (index as f32 * 80.0)),
            )
            .frame(
                egui::Frame::window(&ctx.style())
                    .fill(egui::Color32::from_rgba_premultiplied(40, 40, 60, 240)),
            )
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                ...
                let progress =
                    (notification.duration - notification.timestamp) / notification.duration;
                ui.add(
