...
pub async fn fetch_osm_features(
    ...
    let overpass: OverpassResponse =
        serde_json::from_str(&text).context("Failed to parse Overpass response")?;
...
fn build_overpass_query(bounds: &GeoBounds, config: &OsmConfig) -> String {
    ...
    if config.show_fields {
        query_parts.push(format!(
            r#"way["landuse"~"farmland|farmyard|orchard|vineyard|meadow|greenhouse"]({bbox});"#
        ));
        query_parts.push(format!(
            r#"relation["landuse"~"farmland|farmyard|orchard|vineyard"]({bbox});"#
        ));
    }

    if config.show_buildings {
        query_parts.push(format!(
            r#"way["building"~"barn|silo|farm|farm_auxiliary|greenhouse|shed"]({bbox});"#
        ));
    ...
    if config.show_roads {
        query_parts.push(format!(
            r#"way["highway"~"track|path|service|unclassified"]({bbox});"#
        ));
    ...
    if config.show_water {
        query_parts.push(format!(
            r#"way["waterway"~"stream|ditch|canal|drain"]({bbox});"#
        ));
...
fn compute_osm_stats(features: &[OsmFeature]) -> OsmStats {
    ...
    for feature in features {
        match feature.feature_type {
            OsmFeatureType::Farmland
            | OsmFeatureType::Farmyard
            | OsmFeatureType::Orchard
            | OsmFeatureType::Vineyard => {
                stats.field_count += 1;
            }
            t if t.is_building() => {
            ...
            OsmFeatureType::Stream
            | OsmFeatureType::Ditch
            | OsmFeatureType::Pond
            | OsmFeatureType::Reservoir
            | OsmFeatureType::IrrigationCanal => {
        ...
        for x in 0..width {
            ...
            if edges[idx] > 0 && !visited[idx] {
                ...
                if boundary_points.len() >= 4 {
                    ...
                    let geo_points: Vec<GeoPoint> = boundary_points
                        .iter()
                        .map(|&(px, py)| {
