...
fn handle_spawn_terrain_event(
    ...
    for event in spawn_events.read() {
        ...
        info!(
            ...
            event.bounds.min_lat, event.bounds.max_lat,
            event.bounds.min_lon, event.bounds.max_lon
        );
        
        // Generate heightmap from elevation tiles
        let heightmap = composite_elevation(
            &event.elevation_tiles,
            event.bounds,
        ...
        let base_texture = create_terrain_texture(
            &texture_pixels,
            config.texture_resolution,
        );
...
fn create_terrain_mesh(
    ...
    for z in 0..resolution {
        for x in 0..resolution {
            ...
            let h_l = if x > 0 { positions[(z * resolution + x - 1) as usize][1] } else { positions[idx][1] };
            let h_r = if x < resolution - 1 { positions[(z * resolution + x + 1) as usize][1] } else { positions[idx][1] };
            let h_d = if z > 0 { positions[((z - 1) * resolution + x) as usize][1] } else { positions[idx][1] };
            let h_u = if z < resolution - 1 { positions[((z + 1) * resolution + x) as usize][1] } else { positions[idx][1] };
...
pub fn create_ndvi_overlay_texture(
    ndvi_pixels: &[u8],
    resolution: u32,
) -> Image {
