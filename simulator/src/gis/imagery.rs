...
pub async fn fetch_imagery_esri(coord: TileCoord) -> Result<ImageryTile> {
    ...
    if !response.status().is_success() {
        return Err(anyhow!("Failed to fetch imagery tile: HTTP {}", response.status()));
    }
...
pub async fn fetch_imagery_osm(coord: TileCoord) -> Result<ImageryTile> {
    ...
    if !response.status().is_success() {
        return Err(anyhow!("Failed to fetch OSM tile: HTTP {}", response.status()));
    }
...
fn decode_png(data: &[u8], coord: TileCoord) -> Result<ImageryTile> {
    ...
    Ok(ImageryTile { coord, width, height, pixels })
}
            ...
            for tile in tiles {
                ...
                if lat >= tile_bounds.min_lat && lat <= tile_bounds.max_lat &&
                   lon >= tile_bounds.min_lon && lon <= tile_bounds.max_lon {
                    ...
                    let tv = ((lat - tile_bounds.min_lat) / (tile_bounds.max_lat - tile_bounds.min_lat)) as f32;
                    ...
                    if src_idx + 3 < tile.pixels.len() && dst_idx + 3 < pixels.len() {
                        pixels[dst_idx..dst_idx + 4].copy_from_slice(&tile.pixels[src_idx..src_idx + 4]);
                    }
                    break;
                }
            }
