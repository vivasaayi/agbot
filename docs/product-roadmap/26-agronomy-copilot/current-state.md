...
- The LLM is a true external boundary behind a mockable interface (deterministic-evidence-grounded RAG). Model/provider choice is deployment configuration recorded on every turn as `model_provider`, `model_id`, and `model_version`; the architecture, evidence contract, refusal behavior, and audit trail are the roadmap contract. Tests run against a deterministic test double.
