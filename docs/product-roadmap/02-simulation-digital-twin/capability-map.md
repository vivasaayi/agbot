...
| Safety parity harness (geofence, altitude, battery, no-fly-zone, abort rules identical to real path) | missing | 6 | CI test that proves each safety rule fires identically in twin and real path; fails on any gap |
| Terrain no-data model (available / missing / stale / synthetic / flat_fallback states) | missing | 4 | Load DEM tiles with explicit state tags; missing tile never silently becomes flat zero |
...
| Trace diff CLI (compares two simulation traces, reports exact divergent fields/steps) | missing | 4 | `agbot-sim diff <trace-a> <trace-b>` reports first divergent step and field name |
