...
- **Next (batch 8 candidates, in order):** (b) climatology/drought wiring: feed derived NDVI series into `index_climatology` + `drought_indices` and expose VCI/VHI rasters via `/api/drought-management`; (c) CHIRPS ingestion + SPI; (d) phenology metrics + rule-based crop/tree/water classification; (e) Sen2Cor containerized L1C->L2A path; (f) Phase 4 (crop-type ML, Sentinel-1 water, dNBR forest loss, HLS).
