...
- **Next (batch 5 candidates, in order):** (a) MapLibre GL JS browse UI served by geo_hub; (b) satellite band download wiring (Earth Search assets → local GeoTIFF → indices → L2 registration); (c) VCI climatology store.
