...
- **Batch 7 — DONE (2026-07-05):** global Web Mercator product tiler. `geo_hub/src/product_tiler.rs`: slippy-tile bounds math, per-pixel Mercator->WGS84->UTM inverse chain (reuses `utm.rs`), nearest sampling with nodata/out-of-grid/out-of-UTM-domain reason counts, deterministic per-kind colormaps (vegetation/water/condition ramps over fixed domains), PNG encode; WGS84-envelope fast path for disjoint tiles. Route `GET /api/catalog/products/:id/tiles/:z/:x/:y.png` (`routes/product_tiles.rs`): GeoTIFF-artifact products only, render on blocking pool, disk cache keyed by content-addressed product id. STAC items with GeoTIFF artifacts gain a `tiles_web` asset; `/browse` uses it as a true MapLibre `raster` source (bounds from item bbox), stitch path kept as fallback for legacy scene-local PNG tiles. 11 unit + 3 route tests; stac/browse/satellite/catalog suites green.
