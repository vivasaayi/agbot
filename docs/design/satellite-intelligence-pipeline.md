...
- **Next (batch 6 candidates, in order):** (a) satellite band download wiring (Earth Search assets → local GeoTIFF → indices → L2 registration); (b) VCI climatology store (may be in flight in `post_processor`); (c) global-TMS (Web Mercator) product tiler so `/browse` can switch to a true raster tile source.
