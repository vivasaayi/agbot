...
The workspace crates are: `mission_planner`, `mission_control`, `multi_drone_control`, `sensor_collector`, `data_collector`, `sensor_overlay_engine`, `imagery_processor`, `lidar_mapper`, `post_processor`, `geo_hub`, `geo_viewer`, `ground_station_ui`, and `shared`. There is also a C++ `flight_sim_cpp` component, which is the canonical interactive flight simulator (the former Rust/Bevy `simulator` and `drone_simulator` crates were removed in its favor).
...
- Start from `docs/product-roadmap/` (its `README.md`, `product-doctrine.md`, `requirements-rigor.md`, `implementation-sequencing.md`, and the 12 domain folders), `docs/reference/product-requirements.md`, and the `prompts/` directory (`init.md`, `simulation.md`, `visualization.md`).
...
- If sub-agents are available, use them for backlog triage, Rust crate work, GIS/visualization work, tests, and independent verification. If sub-agents are unavailable, run those passes sequentially.
...
- GIS regression: run `just gis-test` (fast suite) and `just gis-acceptance` (advisor workflow: boundary import → layer serving → annotations → recommendations → reports → exports). Narrow failures with `cargo test -p shared --lib`, `cargo test -p geo_hub --tests --lib`, `cargo test -p geo_viewer`.
- Full workspace tests: `cargo test` or `just test`.
- Formatting and lint: `cargo fmt` and `cargo clippy -- -D warnings` (or `just fmt` / `just clippy`).
...
- If the visible token count is high, roughly above 120k tokens, or Claude enters a long context-compaction phase, stop doing new implementation work.
